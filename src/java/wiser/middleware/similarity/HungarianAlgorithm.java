...
/**
 * Classe di utilit&agrave; che implementa l'algoritmo ungherese.
 *
...
public class HungarianAlgorithm {
    ...
    private static double findLargest(double[][] array) {
        double largest = 0;
    ...
    public static double hgAlgorithm(double[][] array, String sumType) {
        ...
        if (sumType.equalsIgnoreCase("max")) //Then array is weight array. Must change to cost.
        {
            ...
            for (double[] cost1 : cost) //Generate cost by subtracting.
            {
                for (int j = 0; j < cost1.length; j++) {
                    cost1[j] = (maxWeight - cost1[j]);
                }
            }
        }
        ...
        double sum = 0;
        for (int[] assignment1 : assignment) {
            sum = sum + array[assignment1[0]][assignment1[1]];
        }
    ...
    private static int hg_step1(int step, double[][] cost) {
        ...
        for (double[] cost1 : cost) {
            minval = cost1[0];
            for (int j = 0; j < cost1.length; j++) //1st inner loop finds min val in row.
            {
                if (minval > cost1[j]) {
                    minval = cost1[j];
                }
            }
            for (int j = 0; j < cost1.length; j++) //2nd inner loop subtracts it.
            {
                cost1[j] = cost1[j] - minval;
            }
        }
    ...
    private static int hg_step3(int step, int[][] mask, int[] colCover) {
        ...
        for (int[] mask1 : mask) //Cover columns of starred zeros.
        {
